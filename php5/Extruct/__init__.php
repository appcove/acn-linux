...
###################################################################################################
class Extruct_NativeToXML_Convertor extends Extruct_NativeToNative_Convertor
{	
	...
	public function Convert($DATA)
	{
		...
		// Takes native PHP struct and converts it to an XML string
		// base class sends to this->_Struct 

		$oXML = parent::Convert($DATA); 
		//return (string) $oXML	
...
/*
	...
	public function _None($oNode, $DATA)
	{
		return new SimpleXMLElement($oNode->Name);
		//return Element(oNode.Name)
	}
	...
	public function _Bool($oNode, $DATA)
	{
		...
		$DATA = parent::_Int($oNode, $DATA);
		$elem = new SimpleXMLElement($oNode->Name);
	...
	public function _Unicode($oNode, $DATA)
	{
		$DATA = parent::_Unicode($oNode, $DATA);
		$elem = new SimpleXMLElement($oNode->Name);
		$elem->text = (string) $DATA;
		//return elem
...
*/
	...
	public function _Struct($oNode, $DATA)
	{
		...
		$DATA = parent::_Struct($oNode, $DATA);
		$elem = array($oNode->Name);
		...
		catch(Exception $e)
		{
			if(Extruct::$Debug)
			throw $e;
			throw new Extruct_ConversionError($oNode, $DATA, get_class($e) . ': ' . $e->getMessage());
		}
